...
export class BlockchainScoreBoard {
    ...
    private createUI(): void {
      ...
      <div class="blockchain-scoreboard">
        <h2>Статистика турнира в блокчейне</h2>
        
        <div class="connection-section">
          <button id="connect-wallet-btn" class="btn btn-primary">Подключить кошелек</button>
          <span id="connection-status" class="status">Не подключен</span>
        </div>
        
        <div class="contract-section">
          <div class="input-group">
            <input type="text" id="contract-address-input" class="form-control" placeholder="Введите адрес контракта">
            <button id="apply-contract-btn" class="btn btn-secondary">Применить</button>
          </div>
          <button id="deploy-contract-btn" class="btn btn-success" disabled>Развернуть новый контракт</button>
        </div>
        
        <div class="player-stats-section">
          <h3>Статистика игроков</h3>
          <div id="player-list-container" class="player-list">
            <p>Подключите кошелек и установите адрес контракта для просмотра статистики.</p>
          </div>
        </div>
      </div>
    `;
        ...
        this.applyContractButton = this.container.querySelector('#apply-contract-btn') as HTMLButtonElement;
    ...
    private initEventListeners(): void {
        ...
        // Обработчик кнопки применения адреса контракта
        this.applyContractButton.addEventListener('click', () => {
        ...
        this.deployContractButton.addEventListener('click', async () => {
            try {
                ...
                this.deployContractButton.textContent = 'Разворачиваем контракт...';
                ...
                if (contractAddress) {
                    ...
                    alert(`Контракт успешно развернут по адресу: ${contractAddress}`);
                }
            } catch (error) {
    ...
    private updateConnectionStatus(address: string): void {
        this.connectionStatus.textContent = `Подключен: ${address.substring(0, 6)}...${address.substring(address.length - 4)}`;
    ...
    private async loadPlayerStats(): Promise<void> {
        try {
            ...
            if (players.length === 0) {
                this.playerListContainer.innerHTML = '<p>Нет данных о счетах игроков.</p>';
                return;
            }

            // Создаем таблицу с данными игроков
            let html = `
    ...
    private subscribeToScoreUpdates(): void {
        ...
        this.blockchainService.subscribeToScoreUpdates((player, score) => {
            console.log(`Обновлен счет игрока ${player}: ${score}`);
            this.loadPlayerStats();
        });
